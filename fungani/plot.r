...
p1_base <- ggplot(dx, aes(x = NI, y = Freq, fill = NI)) +
  geom_col() +
  geom_label(
...
if (length(args) > 3) {
  if (genosize == 100) {
    pp <- (p1 + p2) / plot_spacer() / p3 / p4 +
      ...
      plot_annotation(
        ...
        subtitle = paste("Genome fraction =", genosize, "%,", "ANI threshold =", threshold, "%"),
  ...
  if (genosize == 100) {
    pp <- (p1 + p2) / plot_spacer() / p3 +
      plot_layout(heights = c(8, 0.5, 0.8)) +
      plot_annotation(
        paste0(
          "AVERAGE NUCLEOTIDE IDENTITY = ",
          format(round(mean(ani1) * 100, 2), nsmall = 2), "%"
        ),
        subtitle = paste("Genome fraction =", genosize, "%,", "ANI threshold =", threshold, "%"),
        theme = theme(
